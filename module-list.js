...
readdir("src", (err, res) => {
  if (err) throw err
  let u = {}
  const mods = res.concat(["index.html"])
    .map(r => getMatches(fs.readFileSync(r).toString(), /window\.getModule\(["']([a-z0-9/-]+)["']\)/gmi))
    .reduce((a, b) => a.concat(b), [])
    .filter(m => u[m] ? false : (u[m] = true)) //remove duplicates
  console.log("Module(s): " + mods.join(", "))
  fs.writeFileSync("bundle.js", Buffer.concat([Buffer.from(header), Buffer.from("\n"), Buffer.from(mods.map(r => 'require(' + JSON.stringify(r) + ')').concat([""]).join("\n"))]))
})
